...
def test_merge_enforce_keep_heterogeneous(batch_size):
    ...
    flexible_list = [("flex", 2), ("flex", 3), ("flex", 4), ("flex", 5),
                     ("flex", 6)]
    final_list = _merge_enforce_keep_heterogeneous(keep_list, flexible_list,
                                                   batch_size)
    ...
    if batch_size <= len(keep_list):
        ...
        len_unique_num = len(
            set([item[1] for item in keep_list + flexible_list]))
        assert num_flex == min(batch_size - num_keep,
                               len_unique_num - num_keep)
...
def test_get_pad_size(expected_pad_size):
    ...
    assert get_pad_size(expected_pad_size - 1,
                        expected_pad_size) == expected_pad_size
    assert get_pad_size(expected_pad_size,
                        expected_pad_size) == expected_pad_size
    assert (get_pad_size(expected_pad_size + 1,
                         expected_pad_size) == 2 * expected_pad_size)
...
@pytest.mark.parametrize(
    ...
    ENFORCE_TEST_COMBO)
def test_enforce_heterogeneous_and_size(batch_size, enforce_heterogeneous,
                                        enforce_sizes, seed):
    prompt_max_length = 4096
    prompt_min_length = 64
    dataset = os.environ.get("SHARE_GPT_DATASET_PATH",
    ...
    if batch_size < len(enforce_size_copy):
        with pytest.raises(ValueError,
                           match="num request is smaller than enforce_sizes"):
        ...
        if enforce_size_copy and enforce_heterogeneous:
            ...
            for size in enforce_size_copy:
                assert size in [
                    get_pad_size(item[1]) for item in prompts_and_sizes
                ]
            # check heterogeneous
